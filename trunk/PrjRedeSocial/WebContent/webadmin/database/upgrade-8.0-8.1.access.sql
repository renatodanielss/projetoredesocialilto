...
alter table content add product_stock_amount integer;

alter table content add product_stock_text varchar(250);

alter table content add product_restocked_amount integer;

alter table content add product_lowstock_amount integer;
...
update content set product_stock_amount = cast(product_stock as signed integer) where product_stock is not null and product_stock <> '';
...
alter table content_archive add product_stock_amount integer;

alter table content_archive add product_stock_text varchar(250);

alter table content_archive add product_restocked_amount integer;

alter table content_archive add product_lowstock_amount integer;
...
update content_archive set product_stock_amount = cast(product_stock as signed integer) where product_stock is not null and product_stock <> '';
...
alter table content_public add product_stock_amount integer;

alter table content_public add product_stock_text varchar(250);

alter table content_public add product_restocked_amount integer;

alter table content_public add product_lowstock_amount integer;
...
update content_public set product_stock_amount = cast(product_stock as signed integer) where product_stock is not null and product_stock <> '';
...
alter table orders add createdyear integer;

alter table orders add createdmonth integer;

alter table orders add createdday integer;

alter table orders add createdweek integer;

alter table orders add createdweekday integer;

alter table orders add createdhour integer;

alter table orders add createdmin integer;

alter table orders add createdsec integer;
...
alter table orders add sessionduration integer;
...
alter table orders add locked_user integer;

alter table orders add locked_creator integer;

alter table orders add locked_editor integer;

alter table orders add locked_publisher integer;

alter table orders add locked_administrator integer;

alter table orders add session_entry varchar(250);

alter table orders add affiliate varchar(250);

alter table orders add temp_order_quantity integer;

update orders set temp_order_quantity = cast(order_quantity as integer) where order_quantity is not null and order_quantity <> '';

alter table orders drop column order_quantity;

alter table orders add order_quantity integer;
...
update orders set temp_order_subtotal = cast(order_subtotal as decimal(20,2)) where order_subtotal is not null and order_subtotal <> '';
...
update orders set temp_discount_total = cast(discount_total as decimal(20,2)) where discount_total is not null and discount_total <> '';
...
update orders set temp_shipping_total = cast(shipping_total as decimal(20,2)) where shipping_total is not null and shipping_total <> '';
...
update orders set temp_tax_total = cast(tax_total as decimal(20,2)) where tax_total is not null and tax_total <> '';
...
update orders set temp_order_total = cast(order_total as decimal(20,2)) where order_total is not null and order_total <> '';
...
alter table orderitems add temp_item_quantity integer;

update orderitems set temp_item_quantity = cast(item_quantity as integer) where item_quantity is not null and item_quantity <> '';

alter table orderitems drop column item_quantity;

alter table orderitems add item_quantity integer;
...
update orderitems set temp_item_total = cast(item_total as decimal(20,2)) where item_total is not null and item_total <> '';
...
update orderitems set temp_discount_total = cast(discount_total as decimal(20,2)) where discount_total is not null and discount_total <> '';
...
update orderitems set temp_shipping_total = cast(shipping_total as decimal(20,2)) where shipping_total is not null and shipping_total <> '';
...
update orderitems set temp_tax_total = cast(tax_total as decimal(20,2)) where tax_total is not null and tax_total <> '';
...
alter table users add product_points integer;

alter table discounts add user_points_min integer;

alter table discounts add user_points_cost integer;

alter table shipping add user_points_min integer;

alter table shipping add user_points_cost integer;
