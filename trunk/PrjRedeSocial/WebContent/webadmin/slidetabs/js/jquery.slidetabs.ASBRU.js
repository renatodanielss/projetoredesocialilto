...
(function($) {
	...
	function SlideTabs($container, conf) {
		...
		this.tabs_init();
// MODIFIED BY ASBRU - FIX: "find on page" breaks tabs display - make non-active tabs content hidden
		this.$views.css('visibility', 'hidden');
...
// MODIFIED BY ASBRU - FIX: tab between inputs/selects/textareas on different tabs breaks tabs display
// MODIFIED BY ASBRU - FIX: only tab between inputs/selects/textareas on the same tab
		$(document).on('keydown', 'input,select,textarea', function(evt) {
			var keycode = evt.keyCode;
			if (keycode == 9) {
				var mytab = $(this).closest("div." + conf.classView);
				if (mytab.length) {
					var myinput = false;
					var myfirstinput = false;
					var mylastinput = false;
					var myinputs = mytab.find("input,select,textarea");
					for (var i=0; i<myinputs.length; i++) {
						if (myinputs[i].type.toLowerCase() != "hidden") {
							if ($(this)[0] == myinputs[i]) {
								myinput = i;
							}
							if (myfirstinput === false) {
								myfirstinput = i;
							}
							mylastinput = i;
						}
					}
					if ((! evt.shiftKey) && (myinput !== false) && (mylastinput !== false) && (myinput >= mylastinput)) {
						evt.stopPropagation();
						evt.preventDefault();
						if (myfirstinput !== false) {
							myinputs[myfirstinput].focus();
						}
					}
					if ((evt.shiftKey) && (myinput !== false) && (myfirstinput !== false) && (myinput <= myfirstinput)) {
						evt.stopPropagation();
						evt.preventDefault();
						if (mylastinput !== false) {
							myinputs[mylastinput].focus();
						}
					}
				}
			}
...
// MODIFIED BY ASBRU - trigger initial delayed resize in case of delayed page/DOM initialization
			var self = this;
			if (self.conf.responsive == true) {
				setTimeout(function(){
					self.tabs_resize();
				}, self.conf.responsiveResizeSpeed);
			}
...
// MODIFIED BY ASBRU - to resize tabs on DOM change
			...
			if (self.conf.responsive == true) {
				...
				$(window).resize(function() {
					...
					resizeTimer = setTimeout(function() {
						...
						// Change the content container(s) height (if this is not a parent instance)
						if (self.conf.autoHeight == true && !self.isParent) { self.content_setHeight(true); }
// MODIFIED BY ASBRU - MSIE may need more than 100ms before resize
//					}, 100);
					}, self.conf.responsiveResizeSpeed);
				});
			}
...
// MODIFIED BY ASBRU - FIX: "find on page" breaks tabs display - make all tabs content visible while animating
			self.$views.css('visibility', 'visible');
...
// MODIFIED BY ASBRU - FIX: "find on page" breaks tabs display - make non-active tabs content hidden
				self.$views.css('visibility', 'hidden');
...
// MODIFIED BY ASBRU - FIX: "find on page" breaks tabs display - make non-active tabs content hidden
		...
		setOptions: function(new_config) {
			...
			// Set/unset scrolling 
// MODIFIED BY ASBRU - FIX: Error: TypeError: self.$tabs.mousewheel is not a function
//			if (this.conf.tabsScroll == true) { 
			if ($.fn.mousewheel && this.conf.tabsScroll) { 
				var self = this;
				
				self.$tabs.mousewheel(function(event, delta) {
// MODIFIED BY ASBRU - FIX? duplicate slightly different program code from elsewhere in this program file
...
// MODIFIED BY ASBRU - FIX: Error: TypeError: this.content_setContentHeight is not a function
//			if (new_config.autoHeight == true) { this.content_setContentHeight(); }
			if (new_config.autoHeight == true) { this.content_setHeight(); }
