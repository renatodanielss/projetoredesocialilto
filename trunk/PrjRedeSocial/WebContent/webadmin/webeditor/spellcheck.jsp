...
<%
	if ((request.getMethod().equals("POST")) && (! spellcheckCommand.equals(""))) {
		if ((request.getParameter("dictionary") != null) && (! request.getParameter("dictionary").equals(""))) {
			spellcheckParameters = spellcheckParameters + " " + spellcheckDictionary + " " + request.getParameter("dictionary").replaceAll("[^-_a-zA-Z0-9]", "");
		}
		...
		try {
			java.io.File f = new java.io.File(spellcheckCommand);
			if (! f.exists()) {
				%><%= "ERROR: spell check command not found: " + spellcheckCommand %><%
			}
			...
			while ((myline = execCommand.readLine()) != null) {
				...
				if ((! myline.equals("")) && ((myline.substring(0,1).equals("&")) || (myline.substring(0,1).equals("?")) || (myline.substring(0,1).equals("#")))) {
