...
<%!
	private HashMap getFileupload(HttpServletRequest request, String filepathname) {
		String blocked_files = "\\.(asp|aspx|cfm|jsp|php|php3|php4|cgi|sh|pl)$";
		try {
			...
			while ((totalBytesRead < contentLength) && (lastBytesRead > -1)) {
				...
				if (totalBytesRead == 0) {
					...
					while (parameternames.hasMoreElements()) {
						...
						myfileupload.put(inputname, inputvalue);
					}
				}
				...
				if (expected.equals("")) {
					...
					if (myforminput.get("filename") != null) {
						...
						String filename = ("" + myforminput.get("filename")).replaceAll("[^-_.a-zA-Z0-9]", "_");
			...
			if (myfileupload.get("file") != null) {
				myfileupload.put("title", ("" + myfileupload.get("title")).replaceAll("[']", "_"));
				return myfileupload;
			} else {
				return new HashMap();
			}
