...
//		columnSpecs : [,,,,,{type:'date'},{type:'date'}],
	...
	buffer=new Rico.Buffer.AjaxSQL(document.location.protocol + '//' + document.location.host + '/<%= mytext.display("adminpath") %>/orders/search.xml.jsp?' + Math.random() + '&redirect=<%= URLEncoder.encode(myrequest.getRequestURI() + "?" + myrequest.getQueryString(), myconfig.get(db, "charset")) %>&', {
