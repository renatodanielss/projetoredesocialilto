...
// Added by Asbru Ltd. - MSIE 11 handling
    this.wheelEvent=(Rico.isIE || Rico.isOpera || Rico.isWebKit || Rico.isIE11) ? 'mousewheel' : 'DOMMouseScroll';
 ...
 */
  ...
  remainingHt: function() {
	...
	// Added by Asbru Ltd. - adjustHeight option
	if (this.options.adjustHeight) margin -= this.options.adjustHeight;
...
// Added by Asbru Ltd. - jQuery event wheelDelta handling
    } else if (e.originalEvent && e.originalEvent.wheelDelta) {
      if (Rico.isOpera)
        delta = e.originalEvent.wheelDelta/120;
      else if (Rico.isWebKit)
        delta = -e.originalEvent.wheelDelta/12;
      else
        delta = -e.originalEvent.wheelDelta/120;
    } else if (e.originalEvent && e.originalEvent.detail) {
      delta = e.originalEvent.detail/3; /* Mozilla/Gecko */
...
fixHeaders: function(prefix, iconsfirst) {
  // Added by Asbru Ltd. - disable sortable if class="unsortable"
  if (this.hdrCellDiv && this.hdrCellDiv.parentNode && this.hdrCellDiv.parentNode.parentNode && this.hdrCellDiv.parentNode.parentNode.className && this.hdrCellDiv.parentNode.parentNode.className.indexOf("unsortable")>=0) {
...
_format: function(v) {
  // Added by Asbru Ltd. - Mozilla Firefox 20 may not display CDATA correctly - but display the "]]>"
  //return v;
  return v.replace(/^([\s\S]*?)<!\[CDATA\[([\s\S]*)\]\]>([\s\S]*?)$/g, "$1$2$3");
},
