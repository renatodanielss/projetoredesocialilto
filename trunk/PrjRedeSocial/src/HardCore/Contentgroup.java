...
public class Contentgroup {
	...
	public Contentgroup(Contentgroup clonegroup) {
		init();
		if (clonegroup != null) {
			contentgroup = clonegroup.getContentgroup();
			id = "";
			parentgroup = clonegroup.getParentgroup();
			users_users = clonegroup.getUsersUsers();
			users_type = clonegroup.getUsersType();
			users_group = clonegroup.getUsersGroup();
			creators_users = clonegroup.getCreatorsUsers();
			creators_type = clonegroup.getCreatorsType();
			creators_group = clonegroup.getCreatorsGroup();
			developers_users = clonegroup.getDevelopersUsers();
			developers_type = clonegroup.getDevelopersType();
			developers_group = clonegroup.getDevelopersGroup();
			editors_users = clonegroup.getEditorsUsers();
			editors_type = clonegroup.getEditorsType();
			editors_group = clonegroup.getEditorsGroup();
			publishers_users = clonegroup.getPublishersUsers();
			publishers_type = clonegroup.getPublishersType();
			publishers_group = clonegroup.getPublishersGroup();
			administrators_users = clonegroup.getAdministratorsUsers();
			administrators_type = clonegroup.getAdministratorsType();
			administrators_group = clonegroup.getAdministratorsGroup();
			doctype = clonegroup.getDocType();
			template = clonegroup.getTemplate();
			stylesheet = clonegroup.getStyleSheet();
			title_prefix = clonegroup.getTitlePrefix();
			title_suffix = clonegroup.getTitleSuffix();
			user = clonegroup.getUser();
			creator = clonegroup.getCreator();
			editor = clonegroup.getEditor();
			publisher = clonegroup.getPublisher();
			developer = clonegroup.getDeveloper();
			administrator = clonegroup.getAdministrator();
		}
	}
	public Contentgroup(Productgroup clonegroup) {
		init();
		if (clonegroup != null) {
			contentgroup = clonegroup.getProductgroup();
			id = "";
			parentgroup = clonegroup.getParentgroup();
			users_users = clonegroup.getUsersUsers();
			users_type = clonegroup.getUsersType();
	...
	public Contentgroup(Imagegroup clonegroup) {
		init();
		if (clonegroup != null) {
			contentgroup = clonegroup.getImagegroup();
			id = "";
			parentgroup = clonegroup.getParentgroup();
			users_users = clonegroup.getUsersUsers();
			users_type = clonegroup.getUsersType();
	...
	public Contentgroup(Filegroup clonegroup) {
		init();
		if (clonegroup != null) {
			contentgroup = clonegroup.getFilegroup();
			id = "";
			parentgroup = clonegroup.getParentgroup();
			users_users = clonegroup.getUsersUsers();
			users_type = clonegroup.getUsersType();
	...
	public Contentgroup(Linkgroup clonegroup) {
		init();
		if (clonegroup != null) {
			contentgroup = clonegroup.getLinkgroup();
			id = "";
			parentgroup = clonegroup.getParentgroup();
			users_users = clonegroup.getUsersUsers();
			users_type = clonegroup.getUsersType();
	...
	public void read(DB db, String id) {
		...
		if ((id != null) && (! id.equals(""))) {
			String SQL = "select * from contentgroups where id=" + Common.integer(id);
	...
	public void readContentgroup(DB db, String contentgroupvalue) {
		if (db == null) return;
		if (! contentgroupvalue.equals("")) {
			String SQL = "select * from contentgroups where contentgroup='" + Common.SQL_clean(contentgroupvalue) + "'";
			HashMap<String,String> row = db.query_record(SQL);
			if (row != null) {
				getRecord(db, row);
			} else {
	...
	public void getForm(Request request) {
		contentgroup = "" + request.getParameter("contentgroup");
	...
	public void create(DB db) {
		...
		db.insert("contentgroups", mydata);
		Cache.clear(db, "contentgroups");
	}
	...
	public void update(DB db) {
		if (db == null) return;
		if ((id != null) && (! id.equals(""))) {
			Contentgroup old_contentgroup = new Contentgroup();
			old_contentgroup.read(db, id);
			...
			mydata.put("title_prefix", "" + title_prefix);
			mydata.put("title_suffix", "" + title_suffix);
			db.update("contentgroups", "id", id, mydata);

			if ((! old_contentgroup.getContentgroup().equals("")) && (! contentgroup.equals(""))) {
				db.rename("contentgroups", "parentgroup", old_contentgroup.getContentgroup(), contentgroup);
			}
			Cache.clear(db, "contentgroups");
		}
	}
	...
	public void renameUsergroup(DB db, String old_usergroup, String new_usergroup) {
		db.rename("contentgroups", "users_group", old_usergroup, new_usergroup);
		db.rename("contentgroups", "creators_group", old_usergroup, new_usergroup);
		db.rename("contentgroups", "developers_group", old_usergroup, new_usergroup);
		db.rename("contentgroups", "editors_group", old_usergroup, new_usergroup);
		db.rename("contentgroups", "publishers_group", old_usergroup, new_usergroup);
		db.rename("contentgroups", "administrators_group", old_usergroup, new_usergroup);
		Cache.clear(db, "contentgroups");
	}
	...
	public void renameUsertype(DB db, String old_usertype, String new_usertype) {
		db.rename("contentgroups", "users_type", old_usertype, new_usertype);
		db.rename("contentgroups", "creators_type", old_usertype, new_usertype);
		db.rename("contentgroups", "developers_type", old_usertype, new_usertype);
		db.rename("contentgroups", "editors_type", old_usertype, new_usertype);
		db.rename("contentgroups", "publishers_type", old_usertype, new_usertype);
		db.rename("contentgroups", "administrators_type", old_usertype, new_usertype);
		Cache.clear(db, "contentgroups");
	}
	...
	public static String select_options(String selectedlist, DB db, Configuration config, Session session, String suffix) {
		...
		Contentgroup mycontentgroups = new Contentgroup();
		mycontentgroups.records(db, SQL);
		while (mycontentgroups.records(db, "")) {
			if ((! config.get(db, "hide_accessrestricted_menu_items").equals("yes")) || mycontentgroups.getAccessRestrictions(session.get("administrator"), session.get("userid"), session.get("username"), session.get("usertype"), session.get("usergroup"), session.get("usertypes"), session.get("usergroups"), db, config)) {
				String name = "" + mycontentgroups.getContentgroup() + suffix;
				String value = "" + mycontentgroups.getContentgroup();
	...
	public static String dtree_options(DB db, String pre, String mid, String post) {
		return Common.output_column_X2(db, "contentgroups", "contentgroup", pre, mid, post);
	}
	...
	public boolean getAccessRestrictions(String session_administrator, String session_userid, String session_username, String session_usertype, String session_usergroup, String session_usertypes, String session_usergroups, DB db, Configuration config) {
		if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":administrator:"+session_administrator + ":userid:"+session_userid + ":username:"+session_username + ":usertype:"+session_usertype + ":usergroup:"+session_usergroup + ":usertypes:"+session_usertypes + ":usergroups:"+session_usergroups); }
		user = true;
		creator = false;
		editor = false;
		publisher = false;
		...
		if ((config.get(db, "use_accessrestrictions").equals("users")) || (config.get(db, "use_accessrestrictions").equals("all"))) {
			...
			if ((users_type.equals("*")) && (session_username.equals(""))) {
				user = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":users_type:"+users_type); }
				...
				if ((users_group.equals("*")) && (session_username.equals(""))) {
					user = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":users_group:"+users_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((users_type.equals("*")) || (users_group.equals("*"))) && (! session_username.equals(""))) {
						user = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":users_group:"+users_group + ":users_type:"+users_type); }
					}
				...
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! users_users.equals("")) {
						if (session_userid.equals("")) {
							user = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":users_users:"+users_users); }
		...
		if (config.get(db, "use_accessrestrictions").equals("all")) {
			...
			if ((editors_type.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":editors_type:"+editors_type); }
			...
			if ((editors_group.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":editors_group:"+editors_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((editors_type.equals("*")) || (editors_group.equals("*"))) && (! session_username.equals(""))) {
					editor = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":editors_group:"+editors_group + ":editors_type:"+editors_type); }
				}
			...
			if (editor) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! editors_users.equals("")) {
						if (session_userid.equals("")) {
							editor = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":editors_users:"+editors_users); }
			...
			if ((creators_type.equals("*")) && (session_username.equals(""))) {
				creator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":creators_type:"+creators_type); }
				...
				if ((creators_group.equals("*")) && (session_username.equals(""))) {
					creator = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":creators_group:"+creators_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((creators_type.equals("*")) || (creators_group.equals("*"))) && (! session_username.equals(""))) {
						creator = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":creators_group:"+creators_group + ":creators_type:"+creators_type); }
					}
			...
			if (creator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! creators_users.equals("")) {
						if (session_userid.equals("")) {
							creator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":creators_users:"+creators_users); }
			...
			if ((publishers_type.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":publishers_type:"+publishers_type); }
			...
			if ((publishers_group.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((publishers_type.equals("*")) || (publishers_group.equals("*"))) && (! session_username.equals(""))) {
					publisher = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group + ":publishers_type:"+publishers_type); }
				}
			...
			if (publisher) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! publishers_users.equals("")) {
						if (session_userid.equals("")) {
							publisher = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":publishers_users:"+publishers_users); }
			...
			if ((developers_type.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":developers_type:"+developers_type); }
			...
			if ((developers_group.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":developers_group:"+developers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((developers_type.equals("*")) || (developers_group.equals("*"))) && (! session_username.equals(""))) {
					developer = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":developers_group:"+developers_group + ":developers_type:"+developers_type); }
				}
			...
			if (developer) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! developers_users.equals("")) {
						if (session_userid.equals("")) {
							developer = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":developers_users:"+developers_users); }
			...
			if ((administrators_type.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":administrators_type:"+administrators_type); }
			...
			if ((administrators_group.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((administrators_type.equals("*")) || (administrators_group.equals("*"))) && (! session_username.equals(""))) {
					administrator = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group + ":administrators_type:"+administrators_type); }
				}
			...
			if (administrator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! administrators_users.equals("")) {
						if (session_userid.equals("")) {
							administrator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":administrators_users:"+administrators_users); }
		...
		if ((id.equals("")) && (contentgroup.equals("")) && (! session_username.equals(config.get(db, "superadmin")))) {
			editor = false;
			creator = false;
			publisher = false;
			developer = false;
		...
		if (_DEBUG_) { System.out.println("AsbruWCM/Contentgroup.getAccessRestrictions:" + id + ":user:"+user + ":creator:"+creator + ":editor:"+editor + ":publisher:"+publisher + ":developer:"+developer + ":administrator:"+administrator); }
		return editor;
	}
