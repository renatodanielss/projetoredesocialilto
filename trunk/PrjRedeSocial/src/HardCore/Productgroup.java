...
public class Productgroup {
	...
	public void read(DB db, String id) {
		...
		if ((id != null) && (! id.equals(""))) {
			String SQL = "select * from productgroups where id=" + Common.integer(id);
	...
	public void readProductgroup(DB db, String productgroupvalue) {
		if (db == null) return;
		if (! productgroupvalue.equals("")) {
			String SQL = "select * from productgroups where productgroup='" + Common.SQL_clean(productgroupvalue) + "'";
			HashMap<String,String> row = db.query_record(SQL);
			if (row != null) {
				getRecord(db, row);
			} else {
	...
	public void create(DB db) {
		...
		mydata.put("productgroup", "" + productgroup);
		...
		db.insert("productgroups", mydata);
		Cache.clear(db, "productgroups");
	}
	...
	public void update(DB db) {
		if (db == null) return;
		if ((id != null) && (! id.equals(""))) {
			Productgroup old_productgroup = new Productgroup();
			old_productgroup.read(db, id);
			...
			mydata.put("title_prefix", "" + title_prefix);
			mydata.put("title_suffix", "" + title_suffix);
			db.update("productgroups", "id", id, mydata);

			if ((! old_productgroup.getProductgroup().equals("")) && (! productgroup.equals(""))) {
				db.rename("productgroups", "parentgroup", old_productgroup.getProductgroup(), productgroup);
			}
			Cache.clear(db, "productgroups");
		}
	}
	...
	public void renameUsergroup(DB db, String old_usergroup, String new_usergroup) {
		db.rename("productgroups", "users_group", old_usergroup, new_usergroup);
		db.rename("productgroups", "creators_group", old_usergroup, new_usergroup);
		db.rename("productgroups", "developers_group", old_usergroup, new_usergroup);
		db.rename("productgroups", "editors_group", old_usergroup, new_usergroup);
		db.rename("productgroups", "publishers_group", old_usergroup, new_usergroup);
		db.rename("productgroups", "administrators_group", old_usergroup, new_usergroup);
		Cache.clear(db, "productgroups");
	}
	...
	public void renameUsertype(DB db, String old_usertype, String new_usertype) {
		db.rename("productgroups", "users_type", old_usertype, new_usertype);
		db.rename("productgroups", "creators_type", old_usertype, new_usertype);
		db.rename("productgroups", "developers_type", old_usertype, new_usertype);
		db.rename("productgroups", "editors_type", old_usertype, new_usertype);
		db.rename("productgroups", "publishers_type", old_usertype, new_usertype);
		db.rename("productgroups", "administrators_type", old_usertype, new_usertype);
		Cache.clear(db, "productgroups");
	}
	...
	public static String select_options(String selectedlist, DB db, Configuration config, Session session, String suffix) {
		...
		Productgroup myproductgroups = new Productgroup();
		myproductgroups.records(db, SQL);
		while (myproductgroups.records(db, "")) {
			if ((! config.get(db, "hide_accessrestricted_menu_items").equals("yes")) || myproductgroups.getAccessRestrictions(session.get("administrator"), session.get("userid"), session.get("username"), session.get("usertype"), session.get("usergroup"), session.get("usertypes"), session.get("usergroups"), db, config)) {
				String name = "" + myproductgroups.getProductgroup() + suffix;
				String value = "" + myproductgroups.getProductgroup();
	...
	public static String dtree_options(DB db, String pre, String mid, String post) {
		return Common.output_column_X2(db, "productgroups", "productgroup", pre, mid, post);
	}
	...
	public boolean getAccessRestrictions(String session_administrator, String session_userid, String session_username, String session_usertype, String session_usergroup, String session_usertypes, String session_usergroups, DB db, Configuration config) {
		if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":administrator:"+session_administrator + ":userid:"+session_userid + ":username:"+session_username + ":usertype:"+session_usertype + ":usergroup:"+session_usergroup + ":usertypes:"+session_usertypes + ":usergroups:"+session_usergroups); }
		user = true;
		creator = false;
		editor = false;
		publisher = false;
		...
		if ((config.get(db, "use_accessrestrictions").equals("users")) || (config.get(db, "use_accessrestrictions").equals("all"))) {
			...
			if ((users_type.equals("*")) && (session_username.equals(""))) {
				user = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":users_type:"+users_type); }
				...
				if ((users_group.equals("*")) && (session_username.equals(""))) {
					user = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":users_group:"+users_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((users_type.equals("*")) || (users_group.equals("*"))) && (! session_username.equals(""))) {
						user = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":users_group:"+users_group + ":users_type:"+users_type); }
					}
				...
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! users_users.equals("")) {
						if (session_userid.equals("")) {
							user = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":users_users:"+users_users); }
		...
		if (config.get(db, "use_accessrestrictions").equals("all")) {
			...
			if ((editors_type.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":editors_type:"+editors_type); }
			...
			if ((editors_group.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":editors_group:"+editors_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((editors_type.equals("*")) || (editors_group.equals("*"))) && (! session_username.equals(""))) {
					editor = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":editors_group:"+editors_group + ":editors_type:"+editors_type); }
				}
			...
			if (editor) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! editors_users.equals("")) {
						if (session_userid.equals("")) {
							editor = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":editors_users:"+editors_users); }
			...
			if ((creators_type.equals("*")) && (session_username.equals(""))) {
				creator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":creators_type:"+creators_type); }
				...
				if ((creators_group.equals("*")) && (session_username.equals(""))) {
					creator = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":creators_group:"+creators_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((creators_type.equals("*")) || (creators_group.equals("*"))) && (! session_username.equals(""))) {
						creator = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":creators_group:"+creators_group + ":creators_type:"+creators_type); }
					}
			...
			if (creator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! creators_users.equals("")) {
						if (session_userid.equals("")) {
							creator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":creators_users:"+creators_users); }
			...
			if ((publishers_type.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":publishers_type:"+publishers_type); }
			...
			if ((publishers_group.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((publishers_type.equals("*")) || (publishers_group.equals("*"))) && (! session_username.equals(""))) {
					publisher = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group + ":publishers_type:"+publishers_type); }
				}
			...
			if (publisher) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! publishers_users.equals("")) {
						if (session_userid.equals("")) {
							publisher = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":publishers_users:"+publishers_users); }
			...
			if ((developers_type.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":developers_type:"+developers_type); }
			...
			if ((developers_group.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":developers_group:"+developers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((developers_type.equals("*")) || (developers_group.equals("*"))) && (! session_username.equals(""))) {
					developer = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":developers_group:"+developers_group + ":developers_type:"+developers_type); }
				}
			...
			if (developer) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! developers_users.equals("")) {
						if (session_userid.equals("")) {
							developer = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":developers_users:"+developers_users); }
			...
			if ((administrators_type.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":administrators_type:"+administrators_type); }
			...
			if ((administrators_group.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((administrators_type.equals("*")) || (administrators_group.equals("*"))) && (! session_username.equals(""))) {
					administrator = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group + ":administrators_type:"+administrators_type); }
				}
			...
			if (administrator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! administrators_users.equals("")) {
						if (session_userid.equals("")) {
							administrator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":administrators_users:"+administrators_users); }
		...
		if ((id.equals("")) && (productgroup.equals("")) && (! session_username.equals(config.get(db, "superadmin")))) {
			editor = false;
			creator = false;
			publisher = false;
			developer = false;
		...
		if (_DEBUG_) { System.out.println("AsbruWCM/Productgroup.getAccessRestrictions:" + id + ":user:"+user + ":creator:"+creator + ":editor:"+editor + ":publisher:"+publisher + ":developer:"+developer + ":administrator:"+administrator); }
		return editor;
	}
