...
public class UCmaintainDataILiketo {
	...
	public Data doPost(ServletContext server, String DOCUMENT_ROOT, Session mysession, Request myrequest, Response myresponse, Configuration myconfig, DB db, String action) throws Exception {
		...
		if(action.equals("post_collection")){
			//Recupera o nome da cole��o para setar na session
			String nameOfCollection = filepost.getParameter(Str.NAME_COLLECTION); //recupera nome da cole��o
			
			if(nameOfCollection != null && !nameOfCollection.equals("")){ //verifica nome da cole��o
				...
				//n�o existe parametro name_collection
			...
			if(action.equals("post_photo_member")){
				...
				if (rows.get(colid_photo) != null){
					if(!rows.get(colid_photo).equals("avatar_male.png") && !rows.get(colid_photo).equals("avatar_female.png")){
						...
						deleteFileImagePhysically(namePhotoDelete, localImagePath); //m�todo deleta arquivo fisicamente
			...
			if ((! data.getId().equals("")) && data.getEditor()) {
				...
				if(action.equals("post_add_item")){
					...
					for(int i= 0; i < quantItens; i++){
						...
						//Ap�s create e gerar o novo id de cada item pelo sistema, faz update para atualizar o id na database dbcollectionitem
					...
					//faz o fluxo abaixo para cole��o
					data.getFormData(database.columns, filepost);
					data.getFormData(database.columns, myrequest);
					data.getFormData(database.columns, postedfiles);
					data.adjustContent(database.columns);
					...
					//se for action post_collection, faz update do novo id gerado da cole��o
					String idUpdate = data.getId();	//recupera id gerado pelo sistema
					Fileupload filepostUpateID = new Fileupload(null, null, null);
					filepostUpateID.setParameter("id_collection", idUpdate);	//set parametro do cole��o
					
					if ((! data.getId().equals("")) && data.getEditor()) {							
						String created = data.getCreated(database.columns);
						String createdby = data.getCreatedBy(database.columns);
						data.getFormData(database.columns, filepostUpateID);	//filepostUpateID contem o parametro da cole��o
						data.getFormData(database.columns, myrequest);
						data.getFormData(database.columns, postedfiles);
						data.adjustContent(database.columns);
						data.setCreated(database.columns, created, createdby);
					...
					String idUpdate = data.getId();	//recupera id gerado pelo sistema
					Fileupload filepostUpateID = new Fileupload(null, null, null);
					filepostUpateID.setParameter("id_video", idUpdate);	//set parametro do cole��o
					...
					if(action.equals("post_category")){
						...
						System.out.println("Log - Error n�o encontrado parametros de id do name do input hidden para atualizar o id do novo registro na database!");
					}
		...
		if ((redirect.startsWith("http://")) || (redirect.startsWith("https://"))) {
			...
			//se redirect for vazio, n�o redireciona para nenhum lugar! obs:por padr�o estava pagina home codigo abaixo
	...
	/**
	 * M�todo deleta uma lista de arquivos imagens fisicamente, passar uma lista com nomes do arquivo e endere�o de armazenamento
	...
	/**
	 * M�todo deleta um arquivo imagem fisicamente, passar o nome do arquivo e endere�o de armazenamento
	 * @param namePhotoDelete
	 * @param localImagePath
	 */
	...
	/**
	 * M�todo responsavel por deletar uma cole��o e seus itens passando o valor do id da cole��o para deletar
	 * @param mysession
	 * @param db
	...
	public void doDeleteCollection(Session mysession, DB db, String idDeleteCollection) {
			...
			IliketoDAO.deleteDadosIliketo(db, "dbcollection", "id", idDeleteCollection); //m�todo deleta dados na database
			
			//Enquanto existir itens na cole��o
			...
			deleteListFileImagePhysically(listNamesPhotoDelete, localImagePath); //m�todo deleta uma lista de arquivos fisicamente
				
	}
	
	/**
	 * M�todo responsavel por deletar um item por vez passando o valor do id do item para deletar
	 * @param mysession
	 * @param db
	 * @param idDeleteItem
	 */
	public void doDeleteItem(Session mysession, DB db, String idDeleteItem) {
		...
		IliketoDAO.deleteDadosIliketo(db, "dbcollectionitem", "id", idDeleteItem); //m�todo deleta dados na database

	}
	
	/**
	 * M�todo responsavel por deletar um video por vez passando o valor do id do video para deletar
	 * @param mysession
	 * @param db
	 * @param idDeleteVideo
	 */
	public void doDeleteVideo(Session mysession, DB db, String idDeleteVideo) {
		...
		deleteFileImagePhysically(nameVideoDelete, localPath); //m�todo deleta fisicamente
		
		IliketoDAO.deleteDadosIliketo(db, "dbcollectionvideo", "id", idDeleteVideo); //m�todo deleta dados na database

	}

	/**
	 * M�todo deleta membro da database dbmembers e tabela users, deleta todas cole��es e itens. Passar o id do member no parametro
	 * @param mysession
	 * @param db
	...
	public void doDeleteMember(Session mysession, DB db, String idDeleteMember) { 
		...
		//Enquanto existir linhas de registro das cole��es, chama m�todo doDeleteCollection
