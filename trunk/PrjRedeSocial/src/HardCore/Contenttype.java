...
public class Contenttype {
	...
	public Contenttype(Contenttype clonetype) {
		init();
		if (clonetype != null) {
			contenttype = clonetype.getContenttype();
			id = "";
			parenttype = clonetype.getParenttype();
			users_users = clonetype.getUsersUsers();
			users_type = clonetype.getUsersType();
			users_group = clonetype.getUsersGroup();
			creators_users = clonetype.getCreatorsUsers();
			creators_type = clonetype.getCreatorsType();
			creators_group = clonetype.getCreatorsGroup();
			developers_users = clonetype.getDevelopersUsers();
			developers_type = clonetype.getDevelopersType();
			developers_group = clonetype.getDevelopersGroup();
			editors_users = clonetype.getEditorsUsers();
			editors_type = clonetype.getEditorsType();
			editors_group = clonetype.getEditorsGroup();
			publishers_users = clonetype.getPublishersUsers();
			publishers_type = clonetype.getPublishersType();
			publishers_group = clonetype.getPublishersGroup();
			administrators_users = clonetype.getAdministratorsUsers();
			administrators_type = clonetype.getAdministratorsType();
			administrators_group = clonetype.getAdministratorsGroup();
			doctype = clonetype.getDocType();
			template = clonetype.getTemplate();
			stylesheet = clonetype.getStyleSheet();
			title_prefix = clonetype.getTitlePrefix();
			title_suffix = clonetype.getTitleSuffix();
			user = clonetype.getUser();
			creator = clonetype.getCreator();
			editor = clonetype.getEditor();
			publisher = clonetype.getPublisher();
			developer = clonetype.getDeveloper();
			administrator = clonetype.getAdministrator();
		}
	}
	public Contenttype(Producttype clonetype) {
		init();
		if (clonetype != null) {
			contenttype = clonetype.getProducttype();
			id = "";
			parenttype = clonetype.getParenttype();
			users_users = clonetype.getUsersUsers();
			users_type = clonetype.getUsersType();
	...
	public Contenttype(Imagetype clonetype) {
		init();
		if (clonetype != null) {
			contenttype = clonetype.getImagetype();
			id = "";
			parenttype = clonetype.getParenttype();
			users_users = clonetype.getUsersUsers();
			users_type = clonetype.getUsersType();
	...
	public Contenttype(Filetype clonetype) {
		init();
		if (clonetype != null) {
			contenttype = clonetype.getFiletype();
			id = "";
			parenttype = clonetype.getParenttype();
			users_users = clonetype.getUsersUsers();
			users_type = clonetype.getUsersType();
	...
	public Contenttype(Linktype clonetype) {
		init();
		if (clonetype != null) {
			contenttype = clonetype.getLinktype();
			id = "";
			parenttype = clonetype.getParenttype();
			users_users = clonetype.getUsersUsers();
			users_type = clonetype.getUsersType();
	...
	public void read(DB db, String id) {
		...
		if ((id != null) && (! id.equals(""))) {
			String SQL = "select * from contenttypes where id=" + Common.integer(id);
	...
	public void readContenttype(DB db, String contenttypevalue) {
		if (db == null) return;
		if (! contenttypevalue.equals("")) {
			String SQL = "select * from contenttypes where contenttype='" + Common.SQL_clean(contenttypevalue) + "'";
			HashMap<String,String> row = db.query_record(SQL);
			if (row != null) {
				getRecord(db, row);
			} else {
	...
	public void create(DB db) {
		...
		db.insert("contenttypes", mydata);
		Cache.clear(db, "contenttypes");
	}
	...
	public void update(DB db) {
		if (db == null) return;
		if ((id != null) && (! id.equals(""))) {
			Contenttype old_contenttype = new Contenttype();
			old_contenttype.read(db, id);
			...
			mydata.put("title_prefix", "" + title_prefix);
			mydata.put("title_suffix", "" + title_suffix);
			db.update("contenttypes", "id", id, mydata);

			if ((! old_contenttype.getContenttype().equals("")) && (! contenttype.equals(""))) {
				db.rename("contenttypes", "parenttype", old_contenttype.getContenttype(), contenttype);
			}
			Cache.clear(db, "contenttypes");
		}
	}
	...
	public void renameUsergroup(DB db, String old_usergroup, String new_usergroup) {
		db.rename("contenttypes", "users_group", old_usergroup, new_usergroup);
		db.rename("contenttypes", "creators_group", old_usergroup, new_usergroup);
		db.rename("contenttypes", "developers_group", old_usergroup, new_usergroup);
		db.rename("contenttypes", "editors_group", old_usergroup, new_usergroup);
		db.rename("contenttypes", "publishers_group", old_usergroup, new_usergroup);
		db.rename("contenttypes", "administrators_group", old_usergroup, new_usergroup);
		Cache.clear(db, "contenttypes");
	}
	...
	public void renameUsertype(DB db, String old_usertype, String new_usertype) {
		db.rename("contenttypes", "users_type", old_usertype, new_usertype);
		db.rename("contenttypes", "creators_type", old_usertype, new_usertype);
		db.rename("contenttypes", "developers_type", old_usertype, new_usertype);
		db.rename("contenttypes", "editors_type", old_usertype, new_usertype);
		db.rename("contenttypes", "publishers_type", old_usertype, new_usertype);
		db.rename("contenttypes", "administrators_type", old_usertype, new_usertype);
		Cache.clear(db, "contenttypes");
	}
	...
	public static String select_options(String selectedlist, DB db, Configuration config, Session session, String suffix) {
		...
		Contenttype mycontenttypes = new Contenttype();
		mycontenttypes.records(db, SQL);
		while (mycontenttypes.records(db, "")) {
			if ((! config.get(db, "hide_accessrestricted_menu_items").equals("yes")) || mycontenttypes.getAccessRestrictions(session.get("administrator"), session.get("userid"), session.get("username"), session.get("usertype"), session.get("usergroup"), session.get("usertypes"), session.get("usergroups"), db, config)) {
				String name = "" + mycontenttypes.getContenttype() + suffix;
				String value = "" + mycontenttypes.getContenttype();
	...
	public static String dtree_options(DB db, String pre, String mid, String post) {
		return Common.output_column_X2(db, "contenttypes", "contenttype", pre, mid, post);
	}
	...
	public boolean getAccessRestrictions(String session_administrator, String session_userid, String session_username, String session_usertype, String session_usergroup, String session_usertypes, String session_usergroups, DB db, Configuration config) {
		if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":administrator:"+session_administrator + ":userid:"+session_userid + ":username:"+session_username + ":usertype:"+session_usertype + ":usergroup:"+session_usergroup + ":usertypes:"+session_usertypes + ":usergroups:"+session_usergroups); }
		user = true;
		creator = false;
		editor = false;
		publisher = false;
		...
		if ((config.get(db, "use_accessrestrictions").equals("users")) || (config.get(db, "use_accessrestrictions").equals("all"))) {
			...
			if ((users_type.equals("*")) && (session_username.equals(""))) {
				user = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":users_type:"+users_type); }
				...
				if ((users_group.equals("*")) && (session_username.equals(""))) {
					user = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":users_group:"+users_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((users_type.equals("*")) || (users_group.equals("*"))) && (! session_username.equals(""))) {
						user = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":users_group:"+users_group + ":users_type:"+users_type); }
					}
				...
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! users_users.equals("")) {
						if (session_userid.equals("")) {
							user = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":users_users:"+users_users); }
		...
		if (config.get(db, "use_accessrestrictions").equals("all")) {
			...
			if ((editors_type.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":editors_type:"+editors_type); }
			...
			if ((editors_group.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":editors_group:"+editors_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((editors_type.equals("*")) || (editors_group.equals("*"))) && (! session_username.equals(""))) {
					editor = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":editors_group:"+editors_group + ":editors_type:"+editors_type); }
				}
			...
			if (editor) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! editors_users.equals("")) {
						if (session_userid.equals("")) {
							editor = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":editors_users:"+editors_users); }
			...
			if ((creators_type.equals("*")) && (session_username.equals(""))) {
				creator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":creators_type:"+creators_type); }
				...
				if ((creators_group.equals("*")) && (session_username.equals(""))) {
					creator = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":creators_group:"+creators_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((creators_type.equals("*")) || (creators_group.equals("*"))) && (! session_username.equals(""))) {
						creator = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":creators_group:"+creators_group + ":creators_type:"+creators_type); }
					}
			...
			if (creator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! creators_users.equals("")) {
						if (session_userid.equals("")) {
							creator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":creators_users:"+creators_users); }
			...
			if ((publishers_type.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":publishers_type:"+publishers_type); }
			...
			if ((publishers_group.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((publishers_type.equals("*")) || (publishers_group.equals("*"))) && (! session_username.equals(""))) {
					publisher = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group + ":publishers_type:"+publishers_type); }
				}
			...
			if (publisher) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! publishers_users.equals("")) {
						if (session_userid.equals("")) {
							publisher = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":publishers_users:"+publishers_users); }
			...
			if ((developers_type.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":developers_type:"+developers_type); }
			...
			if ((developers_group.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":developers_group:"+developers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((developers_type.equals("*")) || (developers_group.equals("*"))) && (! session_username.equals(""))) {
					developer = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":developers_group:"+developers_group + ":developers_type:"+developers_type); }
				}
			...
			if (developer) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! developers_users.equals("")) {
						if (session_userid.equals("")) {
							developer = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":developers_users:"+developers_users); }
			...
			if ((administrators_type.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":administrators_type:"+administrators_type); }
			...
			if ((administrators_group.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((administrators_type.equals("*")) || (administrators_group.equals("*"))) && (! session_username.equals(""))) {
					administrator = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group + ":administrators_type:"+administrators_type); }
				}
			...
			if (administrator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! administrators_users.equals("")) {
						if (session_userid.equals("")) {
							administrator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":administrators_users:"+administrators_users); }
		...
		if ((id.equals("")) && (contenttype.equals("")) && (! session_username.equals(config.get(db, "superadmin")))) {
			editor = false;
			creator = false;
			publisher = false;
			developer = false;
		...
		if (_DEBUG_) { System.out.println("AsbruWCM/Contenttype.getAccessRestrictions:" + id + ":user:"+user + ":creator:"+creator + ":editor:"+editor + ":publisher:"+publisher + ":developer:"+developer + ":administrator:"+administrator); }
		return editor;
	}
