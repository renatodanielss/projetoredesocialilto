...
public class Version {
	...
	public void read(DB db, String id) {
		...
		if ((id != null) && (! id.equals(""))) {
			...
			if (Pattern.compile("^([0-9]+)$").matcher(id).matches()) {
				SQL = "select * from versions where id=" + Common.integer(id);
			} else {
				SQL = "select * from versions where version='" + Common.SQL_clean(id) + "'";
			}
	...
	public void readVersion(DB db, String versionvalue) {
		if (db == null) return;
		if (! versionvalue.equals("")) {
			String SQL = "select * from versions where version='" + Common.SQL_clean(versionvalue) + "'";
			HashMap<String,String> row = db.query_record(SQL);
			if (row != null) {
				getRecord(db, row);
			} else {
	...
	public void getForm(Request request) {
		...
		currencies = request.getParameter("currencies");
	...
	public void create(DB db) {
		...
		mydata.put("currencies", "" + currencies);
	...
	public void update(DB db) {
		if (db == null) return;
		if ((id != null) && (! id.equals(""))) {
			Version old_version = new Version();
			old_version.read(db, id);
			...
			mydata.put("administrators_type", "" + administrators_type);
			mydata.put("administrators_group", "" + administrators_group);
			db.update("versions", "id", id, mydata);

			if ((! old_version.getVersion().equals("")) && (! version.equals(""))) {
				db.rename("content", "version", old_version.getVersion(), version);
				db.rename("content_archive", "version", old_version.getVersion(), version);
				db.rename("content_public", "version", old_version.getVersion(), version);
			}
		}
	}
	...
	public void renameUsergroup(DB db, String old_usergroup, String new_usergroup) {
		db.rename("versions", "users_group", old_usergroup, new_usergroup);
		db.rename("versions", "creators_group", old_usergroup, new_usergroup);
		db.rename("versions", "developers_group", old_usergroup, new_usergroup);
		db.rename("versions", "editors_group", old_usergroup, new_usergroup);
		db.rename("versions", "publishers_group", old_usergroup, new_usergroup);
		db.rename("versions", "administrators_group", old_usergroup, new_usergroup);
	}



	public void renameUsertype(DB db, String old_usertype, String new_usertype) {
		db.rename("versions", "users_type", old_usertype, new_usertype);
		db.rename("versions", "creators_type", old_usertype, new_usertype);
		db.rename("versions", "developers_type", old_usertype, new_usertype);
		db.rename("versions", "editors_type", old_usertype, new_usertype);
		db.rename("versions", "publishers_type", old_usertype, new_usertype);
		db.rename("versions", "administrators_type", old_usertype, new_usertype);
	}
	...
	public boolean getAccessRestrictions(String session_administrator, String session_userid, String session_username, String session_usertype, String session_usergroup, String session_usertypes, String session_usergroups, DB db, Configuration config) {
		if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":administrator:"+session_administrator + ":userid:"+session_userid + ":username:"+session_username + ":usertype:"+session_usertype + ":usergroup:"+session_usergroup + ":usertypes:"+session_usertypes + ":usergroups:"+session_usergroups); }
		user = true;
		creator = false;
		editor = false;
		publisher = false;
		...
		if ((config.get(db, "use_accessrestrictions").equals("users")) || (config.get(db, "use_accessrestrictions").equals("all"))) {
			...
			if ((users_type.equals("*")) && (session_username.equals(""))) {
				user = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":users_type:"+users_type); }
				...
				if ((users_group.equals("*")) && (session_username.equals(""))) {
					user = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":users_group:"+users_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((users_type.equals("*")) || (users_group.equals("*"))) && (! session_username.equals(""))) {
						user = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":users_group:"+users_group + ":users_type:"+users_type); }
					}
				...
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! users_users.equals("")) {
						if (session_userid.equals("")) {
							user = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":users_users:"+users_users); }
		...
		if (config.get(db, "use_accessrestrictions").equals("all")) {
			...
			if ((editors_type.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":editors_type:"+editors_type); }
			...
			if ((editors_group.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":editors_group:"+editors_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((editors_type.equals("*")) || (editors_group.equals("*"))) && (! session_username.equals(""))) {
					editor = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":editors_group:"+editors_group + ":editors_type:"+editors_type); }
				}
			...
			if (editor) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! editors_users.equals("")) {
						if (session_userid.equals("")) {
							editor = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":editors_users:"+editors_users); }
			...
			if ((creators_type.equals("*")) && (session_username.equals(""))) {
				creator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":creators_type:"+creators_type); }
				...
				if ((creators_group.equals("*")) && (session_username.equals(""))) {
					creator = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":creators_group:"+creators_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((creators_type.equals("*")) || (creators_group.equals("*"))) && (! session_username.equals(""))) {
						creator = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":creators_group:"+creators_group + ":creators_type:"+creators_type); }
					}
			...
			if (creator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! creators_users.equals("")) {
						if (session_userid.equals("")) {
							creator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":creators_users:"+creators_users); }
			...
			if ((publishers_type.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":publishers_type:"+publishers_type); }
			...
			if ((publishers_group.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((publishers_type.equals("*")) || (publishers_group.equals("*"))) && (! session_username.equals(""))) {
					publisher = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group + ":publishers_type:"+publishers_type); }
				}
			...
			if (publisher) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! publishers_users.equals("")) {
						if (session_userid.equals("")) {
							publisher = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":publishers_users:"+publishers_users); }
			...
			if ((developers_type.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":developers_type:"+developers_type); }
			...
			if ((developers_group.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":developers_group:"+developers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((developers_type.equals("*")) || (developers_group.equals("*"))) && (! session_username.equals(""))) {
					developer = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":developers_group:"+developers_group + ":developers_type:"+developers_type); }
				}
			...
			if (developer) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! developers_users.equals("")) {
						if (session_userid.equals("")) {
							developer = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":developers_users:"+developers_users); }
			...
			if ((administrators_type.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":administrators_type:"+administrators_type); }
			...
			if ((administrators_group.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((administrators_type.equals("*")) || (administrators_group.equals("*"))) && (! session_username.equals(""))) {
					administrator = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group + ":administrators_type:"+administrators_type); }
				}
			...
			if (administrator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! administrators_users.equals("")) {
						if (session_userid.equals("")) {
							administrator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":administrators_users:"+administrators_users); }
		...
		if ((id.equals("")) && (version.equals("")) && (! session_username.equals(config.get(db, "superadmin")))) {
			editor = false;
			creator = false;
			publisher = false;
			developer = false;
		...
		if (_DEBUG_) { System.out.println("AsbruWCM/Version.getAccessRestrictions:" + id + ":user:"+user + ":creator:"+creator + ":editor:"+editor + ":publisher:"+publisher + ":developer:"+developer + ":administrator:"+administrator); }
		return editor;
	}
