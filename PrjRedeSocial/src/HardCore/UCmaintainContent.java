...
//TODO: content_id = default user registration content id config option
			...
			if ((! email.equals("")) && (! username.equals("")) && (! password.equals("")) && (! user_id.equals("")) && (error.equals(""))) {
				...
				if (user.getUsername().equals(username)) {
					...
					if (user.getCreator()) {
						...
						user.create(db);	//m�todo cria o usu�rio no Banco de Dados.

						error += "<br>" + text.display("register.created");

						/*Obs: Desativado - Abaixo o Sistema Asbru realiza a cria��o da Personal Page do usu�rio.
		...
		mybody = mybody.replaceAll("@@@emailbackground@@@", myrequest.getProtocol() + myrequest.getServerName() + myrequest.getServerPort() + "/image.jsp?id=535");
