...
public class Linktype {
	...
	public void read(DB db, String id) {
		...
		if ((id != null) && (! id.equals(""))) {
			String SQL = "select * from linktypes where id=" + Common.integer(id);
	...
	public void readLinktype(DB db, String linktypevalue) {
		if (db == null) return;
		if (! linktypevalue.equals("")) {
			String SQL = "select * from linktypes where linktype='" + Common.SQL_clean(linktypevalue) + "'";
			HashMap<String,String> row = db.query_record(SQL);
			if (row != null) {
				getRecord(db, row);
			} else {
	...
	public void create(DB db) {
		...
		db.insert("linktypes", mydata);
		Cache.clear(db, "linktypes");
	}
	...
	public void update(DB db) {
		if (db == null) return;
		if ((id != null) && (! id.equals(""))) {
			Linktype old_linktype = new Linktype();
			old_linktype.read(db, id);
			...
			mydata.put("template", "" + template);
			mydata.put("stylesheet", "" + stylesheet);
			db.update("linktypes", "id", id, mydata);

			if ((! old_linktype.getLinktype().equals("")) && (! linktype.equals(""))) {
				db.rename("linktypes", "parenttype", old_linktype.getLinktype(), linktype);
			}
			Cache.clear(db, "linktypes");
		}
	}
	...
	public void renameUsergroup(DB db, String old_usergroup, String new_usergroup) {
		db.rename("linktypes", "users_group", old_usergroup, new_usergroup);
		db.rename("linktypes", "creators_group", old_usergroup, new_usergroup);
		db.rename("linktypes", "developers_group", old_usergroup, new_usergroup);
		db.rename("linktypes", "editors_group", old_usergroup, new_usergroup);
		db.rename("linktypes", "publishers_group", old_usergroup, new_usergroup);
		db.rename("linktypes", "administrators_group", old_usergroup, new_usergroup);
		Cache.clear(db, "linktypes");
	}
	...
	public void renameUsertype(DB db, String old_usertype, String new_usertype) {
		db.rename("linktypes", "users_type", old_usertype, new_usertype);
		db.rename("linktypes", "creators_type", old_usertype, new_usertype);
		db.rename("linktypes", "developers_type", old_usertype, new_usertype);
		db.rename("linktypes", "editors_type", old_usertype, new_usertype);
		db.rename("linktypes", "publishers_type", old_usertype, new_usertype);
		db.rename("linktypes", "administrators_type", old_usertype, new_usertype);
		Cache.clear(db, "linktypes");
	}
	...
	public static String select_options(DB db, String selected) {
		return Common.select_options(db, "linktypes", "linktype", selected);
	}
	...
	public static String select_options(String selectedlist, DB db, Configuration config, Session session, String suffix) {
		...
		Linktype mylinktypes = new Linktype();
		mylinktypes.records(db, SQL);
		while (mylinktypes.records(db, "")) {
			if ((! config.get(db, "hide_accessrestricted_menu_items").equals("yes")) || mylinktypes.getAccessRestrictions(session.get("administrator"), session.get("userid"), session.get("username"), session.get("usertype"), session.get("usergroup"), session.get("usertypes"), session.get("usergroups"), db, config)) {
				String name = "" + mylinktypes.getLinktype() + suffix;
				String value = "" + mylinktypes.getLinktype();
	...
	public static String dtree_options(DB db, String pre, String mid, String post) {
		return Common.output_column_X2(db, "linktypes", "linktype", pre, mid, post);
	}
	...
	public boolean getAccessRestrictions(String session_administrator, String session_userid, String session_username, String session_usertype, String session_usergroup, String session_usertypes, String session_usergroups, DB db, Configuration config) {
		if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":administrator:"+session_administrator + ":userid:"+session_userid + ":username:"+session_username + ":usertype:"+session_usertype + ":usergroup:"+session_usergroup + ":usertypes:"+session_usertypes + ":usergroups:"+session_usergroups); }
		user = true;
		creator = false;
		editor = false;
		publisher = false;
		...
		if ((config.get(db, "use_accessrestrictions").equals("users")) || (config.get(db, "use_accessrestrictions").equals("all"))) {
			...
			if ((users_type.equals("*")) && (session_username.equals(""))) {
				user = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":users_type:"+users_type); }
				...
				if ((users_group.equals("*")) && (session_username.equals(""))) {
					user = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":users_group:"+users_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((users_type.equals("*")) || (users_group.equals("*"))) && (! session_username.equals(""))) {
						user = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":users_group:"+users_group + ":users_type:"+users_type); }
					}
				...
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! users_users.equals("")) {
						if (session_userid.equals("")) {
							user = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":users_users:"+users_users); }
		...
		if (config.get(db, "use_accessrestrictions").equals("all")) {
			...
			if ((editors_type.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":editors_type:"+editors_type); }
			...
			if ((editors_group.equals("*")) && (session_username.equals(""))) {
				editor = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":editors_group:"+editors_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((editors_type.equals("*")) || (editors_group.equals("*"))) && (! session_username.equals(""))) {
					editor = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":editors_group:"+editors_group + ":editors_type:"+editors_type); }
				}
			...
			if (editor) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! editors_users.equals("")) {
						if (session_userid.equals("")) {
							editor = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":editors_users:"+editors_users); }
			...
			if ((creators_type.equals("*")) && (session_username.equals(""))) {
				creator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":creators_type:"+creators_type); }
				...
				if ((creators_group.equals("*")) && (session_username.equals(""))) {
					creator = false;
					if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":creators_group:"+creators_group); }
				...
				if (config.get(db, "accessrestrictions").equals("or")) {
					if (((creators_type.equals("*")) || (creators_group.equals("*"))) && (! session_username.equals(""))) {
						creator = true;
						if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":creators_group:"+creators_group + ":creators_type:"+creators_type); }
					}
			...
			if (creator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! creators_users.equals("")) {
						if (session_userid.equals("")) {
							creator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":creators_users:"+creators_users); }
			...
			if ((publishers_type.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":publishers_type:"+publishers_type); }
			...
			if ((publishers_group.equals("*")) && (session_username.equals(""))) {
				publisher = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((publishers_type.equals("*")) || (publishers_group.equals("*"))) && (! session_username.equals(""))) {
					publisher = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":publishers_group:"+publishers_group + ":publishers_type:"+publishers_type); }
				}
			...
			if (publisher) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! publishers_users.equals("")) {
						if (session_userid.equals("")) {
							publisher = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":publishers_users:"+publishers_users); }
			...
			if ((developers_type.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":developers_type:"+developers_type); }
			...
			if ((developers_group.equals("*")) && (session_username.equals(""))) {
				developer = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":developers_group:"+developers_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((developers_type.equals("*")) || (developers_group.equals("*"))) && (! session_username.equals(""))) {
					developer = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":developers_group:"+developers_group + ":developers_type:"+developers_type); }
				}
			...
			if (developer) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! developers_users.equals("")) {
						if (session_userid.equals("")) {
							developer = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":developers_users:"+developers_users); }
			...
			if ((administrators_type.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":administrators_type:"+administrators_type); }
			...
			if ((administrators_group.equals("*")) && (session_username.equals(""))) {
				administrator = false;
				if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group); }
			...
			if (config.get(db, "accessrestrictions").equals("or")) {
				if (((administrators_type.equals("*")) || (administrators_group.equals("*"))) && (! session_username.equals(""))) {
					administrator = true;
					if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":administrators_group:"+administrators_group + ":administrators_type:"+administrators_type); }
				}
			...
			if (administrator) {
				if (config.get(db, "use_useraccessrestrictions").equals("yes")) {
					if (! administrators_users.equals("")) {
						if (session_userid.equals("")) {
							administrator = false;
							if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":administrators_users:"+administrators_users); }
		...
		if ((id.equals("")) && (linktype.equals("")) && (! session_username.equals(config.get(db, "superadmin")))) {
			editor = false;
			creator = false;
			publisher = false;
			developer = false;
		...
		if (_DEBUG_) { System.out.println("AsbruWCM/Linktype.getAccessRestrictions:" + id + ":user:"+user + ":creator:"+creator + ":editor:"+editor + ":publisher:"+publisher + ":developer:"+developer + ":administrator:"+administrator); }
		return editor;
	}
