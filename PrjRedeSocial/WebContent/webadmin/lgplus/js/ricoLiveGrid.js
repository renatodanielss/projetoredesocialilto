...
Rico.LiveGrid.prototype = {
  ...
  initialize: function( tableId, visibleRows, totalRows, url, options, ajaxOptions ) {
    ...
    this.options = {
      adjustHeight      : 0,  /* number of pixels to adjust autosize grid height - for example to make content under grid visible */
...
//      this.tabs[i].cellPadding = 0;
  ...
  sizeDivs: function() {
    ...
    this.rowHeight = Math.round(dataHt/this.metaData.getPageSize());
  ...
  createRows: function(numrows) {
    ...
    if (numrows < 0) {
      ...
      availHt += this.options.adjustHeight;
      numrows=0;
  ...
  scrollToRow: function(rowOffset) {
     ...
     this.requestContentRefresh(rowOffset);
  },
