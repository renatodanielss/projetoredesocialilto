...
<%
	...
	if ((! root_path.equals("")) && (! upload_path.equals("")) && (enable_upload.equals("yes"))) {
		...
		if ((! pages_path.equals("")) && (myfileupload.get("section").equals("Pages"))) {
			section_path = pages_path;
			allowed_files = "\\.(" + page_formats.replaceAll(",", "|") + ")$";
		} else if ((! images_path.equals("")) && (myfileupload.get("section").equals("Images"))) {
			section_path = images_path;
			allowed_files = "\\.(" + image_formats.replaceAll(",", "|") + ")$";
		} else if ((! files_path.equals("")) && (myfileupload.get("section").equals("Files"))) {
		...
		if (! section_path.equals("")) {
			if ((myfileupload.get("action").equals("Create")) && (myfileupload.get("title") != null) && (! myfileupload.get("title").equals(""))) {
				...
				java.io.File fh = new java.io.File(root_path + section_path + myfileupload.get("category") + myfileupload.get("title"));
				if ((m.find()) && (! myfileupload.get("file.filename").equals("")) && (! fh.exists())) {
					new java.io.File(upload_path + myfileupload.get("file.filename")).renameTo(fh);
				}
				...
				if (m.find()) {
					if ((myfileupload.get("file.filename") != null) && (! myfileupload.get("file.filename").equals(""))) {
						if (myfileupload.get("old_title").equals(myfileupload.get("title"))) {
							// REPLACE
							java.io.File fh = new java.io.File(root_path + section_path + myfileupload.get("category") + myfileupload.get("old_title"));
							...
							java.io.File fh = new java.io.File(root_path + section_path + myfileupload.get("category") + myfileupload.get("old_title"));
							java.io.File fh2 = new java.io.File(root_path + section_path + myfileupload.get("category") + myfileupload.get("title"));
							if ((m.find()) && (fh.exists() && fh.isFile()) && (! fh2.exists())) {
								fh.delete();
								new java.io.File(upload_path + myfileupload.get("file.filename")).renameTo(fh2);
							}
						}
		...
		if ((myfileupload.get("file.filename") != null) && (! myfileupload.get("file.filename").equals(""))) {
			java.io.File fh = new java.io.File(upload_path + myfileupload.get("file.filename"));
			if (fh.exists()) {
				fh.delete();
			}
		}
...
%>
<%@ include file="hyperlinknotification.jsp.html" %>
